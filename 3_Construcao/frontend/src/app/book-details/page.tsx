...
import {
  BookOpen, Star, MapPin, Calendar, CheckCircle, AlertCircle,
  ChevronLeft, Heart, Share2, Loader2
} from 'lucide-react';
...
export default function BookDetails() {
  ...
  const { data: book, isLoading } = useQuery({
    ...
    queryFn: async () => { const books = await api.entities.Book.filter({ id: bookId }); return books[0]; },
    enabled: !!bookId
  });
  ...
  const { data: existingReservations = [] } = useQuery({
    ...
    queryFn: () => api.entities.Reservation.filter({ book_id: bookId, member_id: user?.email, status: 'active' }),
    enabled: !!bookId && !!user?.email, initialData: []
  });
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card className="border-0 shadow-sm"><CardContent className="p-12 text-center">
          ...
          <Link to={createPageUrl('SearchBooks')}><Button variant="outline" className="mt-4"><ChevronLeft className="w-4 h-4 mr-2" />Voltar à pesquisa</Button></Link>
        </CardContent></Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm">
            <Link to={createPageUrl('SearchBooks')} className="text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-1"><ChevronLeft className="w-4 h-4" />Pesquisa</Link>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-1">
            <div className="sticky top-8">
              ...
              <div className="mt-4 flex gap-2">
                <Button variant="outline" className="flex-1" size="sm"><Heart className="w-4 h-4 mr-2" />Favoritar</Button>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2">
            <div className="flex flex-wrap gap-2 mb-4">
              {book.category && <Badge variant="secondary" className="bg-indigo-50 text-indigo-700">{book.category}</Badge>}
              {book.language && <Badge variant="outline">{book.language === 'pt' ? 'Português' : book.language === 'en' ? 'Inglês' : book.language === 'es' ? 'Espanhol' : book.language}</Badge>}
            </div>
            ...
            <Card className="border-0 shadow-sm mb-6">
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {book.isbn && <div><p className="text-slate-500">ISBN</p><p className="font-medium text-slate-800">{book.isbn}</p></div>}
                  {book.publisher && <div><p className="text-slate-500">Editora</p><p className="font-medium text-slate-800">{book.publisher}</p></div>}
                  {book.publication_year && <div><p className="text-slate-500">Ano de Publicação</p><p className="font-medium text-slate-800">{book.publication_year}</p></div>}
                  {book.edition && <div><p className="text-slate-500">Edição</p><p className="font-medium text-slate-800">{book.edition}</p></div>}
                  {book.pages && <div><p className="text-slate-500">Páginas</p><p className="font-medium text-slate-800">{book.pages}</p></div>}
                  {book.location && <div><p className="text-slate-500">Localização</p><p className="font-medium text-slate-800 flex items-center gap-1"><MapPin className="w-4 h-4 text-indigo-600" />{book.location}</p></div>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                {user && <Button variant="outline" size="sm" onClick={() => setShowReviewDialog(true)}>Avaliar</Button>}
              </CardHeader>
              <CardContent>
                {reviews.length === 0 ? <p className="text-slate-500 text-center py-4">Nenhuma avaliação ainda. Seja o primeiro a avaliar!</p> : (
                  <div className="space-y-4">
                    {reviews.map(review => (
                      <div key={review.id} className="border-b border-slate-100 last:border-0 pb-4 last:pb-0">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-medium">{review.user_name?.charAt(0) || 'U'}</div>
      ...
      <Dialog open={showReserveDialog} onOpenChange={setShowReserveDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => reserveMutation.mutate()} disabled={reserveMutation.isPending}>{reserveMutation.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}Confirmar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showReviewDialog} onOpenChange={setShowReviewDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>Avaliar Livro</DialogTitle></DialogHeader>
          <div className="py-4 space-y-4">
            <div>
              <p className="text-sm text-slate-600 mb-2">Sua avaliação</p>
              <div className="flex gap-1">
                {Array.from({ length: 5 }).map((_, i) => <button key={i} onClick={() => setReviewRating(i + 1)} className="p-1"><Star className={cn("w-8 h-8 transition-colors", i < reviewRating ? "text-amber-400 fill-amber-400" : "text-slate-200 hover:text-amber-200")} /></button>)}
              </div>
            </div>
            <div>
              <p className="text-sm text-slate-600 mb-2">Comentário (opcional)</p>
              <Textarea value={reviewText} onChange={(e) => setReviewText(e.target.value)} placeholder="Conte o que achou do livro..." rows={4} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowReviewDialog(false)}>Cancelar</Button>
            <Button onClick={() => reviewMutation.mutate()} disabled={reviewMutation.isPending || reviewRating === 0}>{reviewMutation.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}Enviar Avaliação</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
