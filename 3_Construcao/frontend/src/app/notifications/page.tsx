...
export default function Notifications() {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (notificationIds: string[]) => {
      for (const id of notificationIds) {
        await api.entities.Notification.update(id, { 
          status: 'read', 
  ...
  const updatePreferencesMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['member-prefs', user?.email] });
  ...
  const getNotificationIcon = (notification) => {
    switch (notification.action_type) {
      ...
      case 'collect_reservation': return <BookOpen className="w-5 h-5 text-emerald-600" />;
      case 'view_loan': return <Clock className="w-5 h-5 text-orange-600" />;
  ...
  const getActionUrl = (notification) => {
    switch (notification.action_type) {
      ...
      case 'collect_reservation': return createPageUrl('MyReservations');
      case 'pay_fine': return createPageUrl('Profile');
      default: return null;
    }
  };
        ...
        <Tabs defaultValue="notifications">
          <TabsList className="mb-6">
            <TabsTrigger value="notifications">
              ...
              Notificações
              {unreadCount > 0 && (
                <Badge className="ml-2 bg-red-100 text-red-700">{unreadCount}</Badge>
              )}
            </TabsTrigger>
            ...
            <div className="flex items-center justify-between mb-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  ...
                  <DropdownMenuItem onClick={() => setFilter('read')}>Lidas</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <Card className="border-0 shadow-sm">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-slate-500">
                    {filter === 'unread' ? 'Você está em dia com todas as suas notificações!' : 'Suas notificações aparecerão aqui.'}
                  </p>
                </CardContent>
              </Card>
            ) : (
                  ...
                  {filteredNotifications.map((notification, index) => (
                    ...
                    >
                      <Card className={cn(
                        ...
                        notification.status !== 'read' && "bg-indigo-50/50 ring-1 ring-indigo-100"
                      )}>
                            ...
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  ...
                                  setSelectedNotifications(selectedNotifications.filter(i => i !== notification.id));
                                }
                              }} 
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-2">
                                  <h3 className={cn(
                                    "text-sm",
                                    notification.status !== 'read' ? "font-semibold text-slate-800" : "font-medium text-slate-600"
                                  )}>
                                    {notification.title}
                                  </h3>
                                ...
                                {notification.action_type !== 'none' && (
                                  <Button variant="link" size="sm" className="h-auto p-0 mt-2 text-indigo-600">
                                    {notification.action_type === 'renew' && 'Renovar agora'}
                                    {notification.action_type === 'pay_fine' && 'Pagar multa'}
                                    {notification.action_type === 'collect_reservation' && 'Ver reserva'}
                                    {notification.action_type === 'view_loan' && 'Ver empréstimo'}
                                  </Button>
                                )}
                              </div>
                            </div>
