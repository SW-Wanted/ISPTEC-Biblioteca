...
export default function ManageFines() {
  ...
  const [activeTab, setActiveTab] = useState<"pending" | "paid" | "waived">(
    "pending",
  );
  ...
  const waiveFinesMutation = useMutation({
    ...
    onError: () => {
      toast.error("Erro ao dispensar multa");
    },
  });
  ...
  const getFineTypeLabel = (type?: string) => {
    switch (type) {
      case "late_return":
        return "Atraso na devolução";
      case "damaged_book":
        return "Livro danificado";
      case "lost_book":
        return "Livro perdido";
      case "locker_overtime":
        return "Excesso de tempo no cacifo";
      case "lost_credential":
        return "Perda de credencial";
      default:
        return type;
    }
  };
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      ...
      case "waived":
        return (
          <Badge className="bg-slate-100 text-slate-700">Dispensado</Badge>
        );
      case "cancelled":
  ...
  const filteredFines = (finesList: FineRow[]) => {
    return finesList.filter((fine) => {
      if (searchQuery) {
        ...
        if (
          !fine.member_name?.toLowerCase().includes(query) &&
          !fine.member_id?.toLowerCase().includes(query)
        )
          return false;
      }
      ...
      <TableCell>
        <div>
          <p className="font-medium text-slate-800">
            {fine.member_name || fine.member_id}
          </p>
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-amber-800 mt-1">
                    {totalPending.toLocaleString("pt-AO", {
                      style: "currency",
                      currency: "AOA",
                    })}
                  </p>
                  <p className="text-xs text-amber-600 mt-1">
                    {pendingFines.length} multa(s)
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-emerald-800 mt-1">
                    {totalPaid.toLocaleString("pt-AO", {
                      style: "currency",
                      currency: "AOA",
                    })}
                  </p>
                  <p className="text-xs text-emerald-600 mt-1">
                    {paidFines.length} pagamento(s)
                  </p>
                </div>
        ...
        <Tabs
          value={activeTab}
          onValueChange={(v) =>
            setActiveTab(v as "pending" | "paid" | "waived")
          }
        >
            ...
            <CardContent className="p-0">
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      filteredFines(pendingFines).map((fine) => (
              ...
              <TabsContent value="paid" className="m-0">
                <Table>
                  ...
                  <TableBody>
                    {filteredFines(paidFines).map((fine) => (
                      <FineRow key={fine.id} fine={fine} />
                    ))}
                  </TableBody>
                </Table>
              ...
              <TabsContent value="waived" className="m-0">
                <Table>
                  ...
                  <TableBody>
                    {filteredFines(waivedFines).map((fine) => (
                      <FineRow key={fine.id} fine={fine} />
                    ))}
                  </TableBody>
                </Table>
            ...
            <div className="p-4 bg-slate-50 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-slate-800">
                  {selectedFine?.amount?.toLocaleString("pt-AO", {
                    style: "currency",
                    currency: "AOA",
                  })}
                </span>
