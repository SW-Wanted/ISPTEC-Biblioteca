...
export default function ManageMembers() {
  ...
  const { data: members = [], isLoading } = useQuery({ queryKey: ['manage-members'], queryFn: () => api.entities.Member.list('-created_date', 200), initialData: [] });

  const getMemberTypeLabel = (type) => { switch (type) { case 'student': return 'Estudante'; case 'teacher': return 'Docente'; case 'staff': return 'Funcionário'; case 'librarian': return 'Bibliotecário'; default: return type; } };
  const getMemberTypeColor = (type) => { switch (type) { case 'student': return 'bg-blue-100 text-blue-700'; case 'teacher': return 'bg-purple-100 text-purple-700'; default: return 'bg-slate-100 text-slate-700'; } };
  const getStatusBadge = (member) => { if (member.is_blocked) return <Badge className="bg-red-100 text-red-700">Bloqueado</Badge>; switch (member.status) { case 'active': return <Badge className="bg-emerald-100 text-emerald-700">Ativo</Badge>; case 'pending': return <Badge className="bg-amber-100 text-amber-700">Pendente</Badge>; default: return null; } };
