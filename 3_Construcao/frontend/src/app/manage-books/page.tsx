  ...
  const createBookMutation = useMutation<void, Error, BookFormData>({
    mutationFn: async (data) => {
      const bookData: Partial<Book> & Record<string, unknown> = {
        ...
        publication_year: data.publication_year ? parseInt(String(data.publication_year), 10) : null,
        ...
        available_copies: Number(data.available_copies) || 1,
      };

      if (isEditing && selectedBook) {
        await api.entities.Book.update(selectedBook.id, bookData);
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8">
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Remover Livro</AlertDialogTitle><AlertDialogDescription>Tem certeza que deseja remover &quot;{selectedBook?.title}&quot;? Esta ação não pode ser desfeita.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => { if (selectedBook) deleteBookMutation.mutate(selectedBook.id); }} className="bg-red-600 hover:bg-red-700">{deleteBookMutation.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}Remover</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
    </div>
  );
}