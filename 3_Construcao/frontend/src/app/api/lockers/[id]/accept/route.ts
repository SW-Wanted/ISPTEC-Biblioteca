	...
	try {
		await prisma.$transaction(async (tx) => {
			...
			if (reservation.locker.status !== LockerStatus.RESERVED) {
				throw new Error("LOCKER_NOT_RESERVED");
			}
			...
			if (activeRental) {
				throw new Error("LOCKER_ALREADY_OCCUPIED");
			}
			...
			const rental = await tx.lockerRental.create({
				data: {
					lockerId: reservation.locker.id,
			...
			await tx.lockerReservation.update({
			...
			await tx.notification.create({
				data: {
					...
					title: "Reserva de cacifo aprovada",
					message: `A tua reserva do cacifo ${reservation.locker.number} foi aprovada. Termino: ${expectedEnd.toLocaleTimeString("pt-AO", { hour: "2-digit", minute: "2-digit" })}.`,
		...
		if (message.includes("LOCKER_NOT_RESERVED")) {
			return NextResponse.json(
				{ error: "LOCKER_NOT_RESERVED" },
				{ status: 409 },
			);
		}
		if (message.includes("LOCKER_ALREADY_OCCUPIED")) {
			return NextResponse.json(
				{ error: "LOCKER_ALREADY_OCCUPIED" },
				{ status: 409 },
			);
		}
