    ...
    await prisma.$transaction(async (tx) => {
      const reservation = await tx.computerReservation.findFirst({
        ...
        select: {
          id: true,
          userId: true,
          computer: { select: { id: true, number: true, status: true } },
        },
      });
      ...
      await tx.notification.create({
        data: {
          ...
          title: "Reserva de computador rejeitada",
          message: `A tua reserva do computador ${reservation.computer.number} foi rejeitada.`,
