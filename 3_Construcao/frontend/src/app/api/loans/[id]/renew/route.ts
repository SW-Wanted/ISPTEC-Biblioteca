...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const preload = await prisma.loan.findUnique({
    where: { id },
    include: { copy: { include: { book: { select: { id: true, title: true } } } }, user: { select: { id: true } } },
  })

  if (!preload) {
    return NextResponse.json({ error: errorMessage("NOT_FOUND") }, { status: 404 })
  }

  if (!canManageLoans(requester.type) && preload.userId !== requester.id) {
    return NextResponse.json({ error: errorMessage("NOT_ALLOWED") }, { status: 403 })
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const loan = await tx.loan.findUnique({
        ...
        include: {
          ...
          user: { select: { id: true, email: true, name: true, type: true, totalFines: true } },
        },
      })
      ...
      const newDueDate = new Date(now.getTime() + loanDays * 24 * 60 * 60 * 1000)
    ...
    const httpStatus = code === "NOT_ALLOWED" ? 403 : code === "NOT_FOUND" ? 404 : 400
    return NextResponse.json({ error: errorMessage(code) }, { status: httpStatus })
  }
}
