...
export default function AdminDashboard() {
  ...
  const { data: books = [] } = useQuery({ queryKey: ['admin-books'], queryFn: () => api.entities.Book.list(), initialData: [] });
  const { data: members = [] } = useQuery({ queryKey: ['admin-members'], queryFn: () => api.entities.Member.list(), initialData: [] });
  const { data: loans = [] } = useQuery({ queryKey: ['admin-loans'], queryFn: () => api.entities.Loan.list(), initialData: [] });
  const { data: reservations = [] } = useQuery({ queryKey: ['admin-reservations'], queryFn: () => api.entities.Reservation.list(), initialData: [] });
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-3"><BarChart3 className="w-7 h-7 text-indigo-600" />Dashboard Administrativo</h1>
            <p className="text-slate-500 mt-1">Visão geral do sistema • {DASHBOARD_TODAY_LABEL}</p>
          </div>
          <Link to={createPageUrl('Reports')}><Button variant="outline"><BarChart3 className="w-4 h-4 mr-2" />Relatórios</Button></Link>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard title="Empréstimos Ativos" value={activeLoans.length} icon={BookMarked} color="bg-gradient-to-br from-indigo-500 to-indigo-600" link="ManageLoans" />
          <StatCard title="Em Atraso" value={overdueLoans.length} icon={AlertTriangle} color="bg-gradient-to-br from-red-500 to-red-600" link="ManageLoans" />
          <StatCard title="Reservas Ativas" value={activeReservations.length} icon={Clock} color="bg-gradient-to-br from-amber-500 to-orange-500" />
          <StatCard title="Multas Pendentes" value={totalPendingFinesAmount.toLocaleString('pt-AO', { style: 'currency', currency: 'AOA' })} icon={Activity} color="bg-gradient-to-br from-purple-500 to-pink-500" />
        </div>
