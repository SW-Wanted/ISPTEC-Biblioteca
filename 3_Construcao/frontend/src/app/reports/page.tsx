...
export default function Reports() {
  ...
  const overdueLoans = loans.filter(
    (l) =>
      l.status === "active" &&
      (l.due_date ? new Date(l.due_date) < new Date() : false),
  ).length;
  ...
  const membersByType = [
    {
      name: "Estudantes",
      value: members.filter((m) => m.member_type === "STUDENT").length,
    },
    {
      name: "Docentes",
      value: members.filter((m) => m.member_type === "TEACHER").length,
    },
    {
      name: "Funcionários",
      value: members.filter((m) => m.member_type === "STAFF").length,
    },
    {
      name: "Bibliotecários",
      value: members.filter((m) => m.member_type === "LIBRARIAN").length,
    },
  ].filter((m) => m.value > 0);

  const topBorrowedBooks = [...books]
    .sort((a, b) => (b.total_loans || 0) - (a.total_loans || 0))
    .slice(0, 10);

  const finesByType = [
    {
      name: "Atraso",
      value: fines.filter((f) => f.type === "late_return").length,
    },
    {
      name: "Livro Danificado",
      value: fines.filter((f) => f.type === "damaged_book").length,
    },
    {
      name: "Livro Perdido",
      value: fines.filter((f) => f.type === "lost_book").length,
    },
    {
      name: "Outros",
      value: fines.filter(
        (f) =>
          !["late_return", "damaged_book", "lost_book"].includes(
            String(f.type ?? ""),
          ),
      ).length,
    },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Análise detalhada do sistema de biblioteca
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          <StatCard
            ...
            value={totalBooks}
            subtitle={`${totalCopies} exemplares`}
            icon={BookOpen}
          ...
          <StatCard
            ...
            value={activeMembers}
            subtitle={`de ${members.length} cadastrados`}
            icon={Users}
          ...
          <StatCard
            ...
            value={activeLoans}
            subtitle={`${overdueLoans} em atraso`}
            icon={Clock}
            color="bg-gradient-to-br from-amber-500 to-orange-500"
          />
          <StatCard
            title="Multas Pendentes"
            value={totalPendingFines.toLocaleString("pt-AO", {
              style: "currency",
              currency: "AOA",
            })}
            subtitle={`${pendingFines.length} multas`}
            icon={DollarSign}
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1 bg-slate-100 rounded-full h-4 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${loans.length > 0 ? (loans.filter((l) => l.status === "returned" && !l.days_overdue).length / loans.filter((l) => l.status === "returned").length) * 100 : 0}%`,
                      }}
                    />
                  </div>
                  <span className="font-semibold text-slate-800">
                    {loans.length > 0
                      ? Math.round(
                          (loans.filter(
                            (l) => l.status === "returned" && !l.days_overdue,
                          ).length /
                            Math.max(
                              1,
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-medium">
                          {availableCopies} de {totalCopies}
                        </span>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="bg-slate-100 rounded-full h-3 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${totalCopies > 0 ? ((totalCopies - availableCopies) / totalCopies) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
              ...
              <Card className="border-0 shadow-sm bg-emerald-50">
                <CardContent className="p-6 text-center">
                  <p className="text-2xl font-bold text-emerald-700">
                    {totalPaidFines.toLocaleString("pt-AO", {
                      style: "currency",
                      currency: "AOA",
                    })}
                  </p>
                  <p className="text-sm text-emerald-600 mt-1">
                    Arrecadadas ({paidFines.length})
                  </p>
                </CardContent>
              </Card>
              <Card className="border-0 shadow-sm bg-slate-50">
                <CardContent className="p-6 text-center">
                  <p className="text-2xl font-bold text-slate-700">
                    {(totalPendingFines + totalPaidFines).toLocaleString(
                      "pt-AO",
